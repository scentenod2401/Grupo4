<?php
define('ZABBIX_URL_SSH', 'http://192.168.14.13/api_jsonrpc.php');
define('ZABBIX_TOKEN_SSH', 'c95d6243e2fc1647d32b807267568fd63220360c8166ebf7bc8502d99dea4872');
define('ZABBIX_HOST_SSH', 'Zabbix-server');
define('SSH_SCRIPT_ID', '9');
define('SSH_CHECK_SCRIPT_ID', '7');

function restart_ssh_handler() {
    ...
    $host_request = array(
        ...
        'params' => array(
            'output' => array('hostid'),
            'filter' => array('host' => array(ZABBIX_HOST_SSH))
        ),
        'id' => 1
    );
    
    $response = wp_remote_post(ZABBIX_URL_SSH, array(
        'headers' => array(
            'Content-Type' => 'application/json-rpc',
            'Authorization' => 'Bearer ' . ZABBIX_TOKEN_SSH
        ),
    ...
    $script_request = array(
        ...
        'params' => array(
            'scriptid' => SSH_SCRIPT_ID,
            'hostid' => $hostid
        ),
        'id' => 2
    );
    
    $response2 = wp_remote_post(ZABBIX_URL_SSH, array(
        'headers' => array(
            'Content-Type' => 'application/json-rpc',
            'Authorization' => 'Bearer ' . ZABBIX_TOKEN_SSH
        ),
    ...
    wp_send_json_success(array('message' => 'SSH reiniciado correctamente'));
    wp_die();
}
add_action('wp_ajax_restart_ssh', 'restart_ssh_handler');

function get_ssh_status() {
    // Intentar obtener desde caché (válido por 10 segundos)
    $cached = get_transient('ssh_status_cache');
    if ($cached !== false) {
        return $cached;
    }
    ...
    if (is_wp_error($response)) {
        ...
        set_transient('ssh_status_cache', $result, 10);
        return $result;
    }
    ...
    if ($value == 'active') {
        ...
        $result = array('status' => 'down', 'message' => 'SSH: Service is not running');
    } else {
...
function ssh_control_styles() {
    echo '<style>
        ...
        .ssh-content.success h2 { color: #155724; }
        .ssh-content.danger h2 { color: #721c24; }
        .ssh-content.info h2 { color: #0c5460; }
        ...
        .restart-ssh-btn:hover:not(:disabled)::before {
            left: 100%;
        }
        
        .restart-ssh-btn:hover:not(:disabled) {
            transform: translateY(-1px);
            box-shadow: 0 4px 12px rgba(220, 53, 69, 0.4);
        }
        
        .restart-ssh-btn:active:not(:disabled) {
            transform: translateY(0);
        }
        
        .restart-ssh-btn:disabled {
            opacity: 0.6;
            cursor: not-allowed;
        }
        ...
        @media (prefers-color-scheme: dark) {
            ...
            .ssh-content.success h2 { color: #88d498; }
            .ssh-content.danger h2 { color: #ff9da0; }
            .ssh-content.info h2 { color: #8edbe6; }
...
add_action('wp_head', 'ssh_control_styles');
...
function check_ssh_status() {
    ...
    $html = '<div id="ssh-status" class="ssh-status-container">';
    
    if ($status['status'] == 'down') {
        $html .= '
        <div class="ssh-card">
            <div class="ssh-header">
                <div class="ssh-icon danger">✕</div>
                <div class="ssh-content danger">
                    <h2>SSH - Inactivo</h2>
                    <p>Servicio detenido</p>
                </div>
            </div>
            <div class="ssh-details danger">
                El servicio SSH no está respondiendo. Se requiere acción inmediata para restaurar el servicio.
            </div>
            <button class="restart-ssh-btn">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <polyline points="23 4 23 10 17 10"></polyline>
                    <path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"></path>
                </svg>
                <span>Reiniciar SSH Ahora</span>
            </button>
            <div class="restart-message"></div>
            <div class="ssh-footer">
                <div class="ssh-update">
        ...
        <div class="ssh-card">
            <div class="ssh-header">
                <div class="ssh-icon success">✓</div>
                <div class="ssh-content success">
                    <h2>SSH - Activo</h2>
                    <p>Servicio operativo</p>
                </div>
            </div>
            <div class="ssh-details success">
                El servicio SSH está funcionando correctamente. No se requiere ninguna acción.
            </div>
            <div class="ssh-footer">
                <div class="ssh-update">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <div class="ssh-card">
            <div class="ssh-header">
                <div class="ssh-icon info">ℹ</div>
                <div class="ssh-content info">
                    <h2>SSH - Estado Desconocido</h2>
                    <p>Información no disponible</p>
                </div>
            </div>
            <div class="ssh-details info">
    ...
    (function() {
        function updateStatus() {
            var container = document.getElementById('ssh-status');
            if (!container) return;
            ...
            fetch(window.location.href)
                ...
                .then(html => {
                    ...
                    var newContent = doc.getElementById('ssh-status');
        ...
        function attachHandlers() {
            var btn = document.querySelector('#ssh-status .restart-ssh-btn');
            if (!btn) return;
            
            btn.onclick = function() {
                var messageDiv = document.querySelector('#ssh-status .restart-message');
                var btnRef = this;
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        btnRef.innerHTML = '<svg width=\"18\" height=\"18\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\"><polyline points=\"23 4 23 10 17 10\"></polyline><path d=\"M20.49 15a9 9 0 1 1-2.12-9.36L23 10\"></path></svg><span>Reiniciar SSH Ahora</span>';
                    }
                });
            };
        }
...
add_shortcode('ssh_check', 'check_ssh_status');
?>
